...
static void on_sort_by_size_reverse_activate (GtkMenuItem *item, gpointer dirview)
{
    myfm_directory_view_set_file_sort_criteria (dirview, MYFM_SORT_LARGEST_FIRST);
}
static void on_sort_by_size_activate (GtkMenuItem *item, gpointer dirview)
{
    myfm_directory_view_set_file_sort_criteria (dirview, MYFM_SORT_LARGEST_LAST);
}
static void on_sort_by_name_z_a_activate (GtkMenuItem *item, gpointer dirview)
{
    myfm_directory_view_set_file_sort_criteria (dirview, MYFM_SORT_NAME_Z_TO_A);
}
static void on_sort_by_name_a_z_activate (GtkMenuItem *item, gpointer dirview)
{
    myfm_directory_view_set_file_sort_criteria (dirview, MYFM_SORT_NAME_A_TO_Z);
}
static void on_sort_by_date_reverse_activate (GtkMenuItem *item, gpointer dirview)
{
    myfm_directory_view_set_file_sort_criteria (dirview, MYFM_SORT_NEWLY_EDITED_LAST);
}
static void on_sort_by_date_activate (GtkMenuItem *item, gpointer dirview)
{
    myfm_directory_view_set_file_sort_criteria (dirview, MYFM_SORT_NEWLY_EDITED_FIRST);
}

static void setup_sort_subitem (GtkWidget *submenu, GtkWidget *subitem,
...
static GtkWidget *myfm_directory_menu_new_submenu_for_sort (MyFMDirectoryMenu *self)
{
    GtkWidget *submenu;

    submenu = gtk_menu_new ();
    setup_sort_subitem (submenu, myfm_utils_new_menu_item ("Last edited (most recent first)", 0, 0),
                        G_CALLBACK (on_sort_by_date_activate), self->dirview);
    setup_sort_subitem (submenu, myfm_utils_new_menu_item ("Last edited (most recent last)", 0, 0),
    ...
    setup_sort_subitem (submenu, myfm_utils_new_menu_item ("Size (smallest to largest)", 0, 0),
                        G_CALLBACK (on_sort_by_size_activate), self->dirview);
    setup_sort_subitem (submenu, myfm_utils_new_menu_item ("Size (largest to smallest)", 0, 0),
                        G_CALLBACK (on_sort_by_size_reverse_activate), self->dirview);

    return submenu;
}

static void myfm_directory_menu_fill (MyFMDirectoryMenu *self)
{
    GtkWidget *sort_item;
...
static void myfm_directory_menu_init (MyFMDirectoryMenu *self)
{
    self->dirview = NULL;
}

static void myfm_directory_menu_class_init (MyFMDirectoryMenuClass *cls)
{
}
