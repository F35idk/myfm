...
static void display_name_callback (GObject *g_file, GAsyncResult *res, gpointer myfm_file)
{
    ...
    /* the myfm_file is ref'd before this callback is invoked, to keep it alive until then.
     * so we unref it after we're done with it here */
...
void myfm_file_init_io_fields_async (MyFMFile *myfm_file)
{
    ...
    /* ensure that the file is kept alive until display_name_callback is invoked, so we
...
MyFMFile *myfm_file_from_g_file (GFile *g_file)
{
    ...
    if (error) {
        g_critical ("unable to set display name on file: %s \n", error->message);
        if (myfm_file->IO_display_name == NULL)
            g_critical ("malloc returned NULL, unable to set temp display name on file \n");
        else
...
// initializes only the fields in a myfm_file that don't require async IO
