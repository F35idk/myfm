...
void myfm_multi_paned_update_size (MyFMMultiPaned *self)
{
    ...
    if (gtk_widget_translate_coordinates (GTK_WIDGET (self->inner_pane),
                                          GTK_WIDGET (self), 0, 0, &new_x, &new_y)) {
        gint prev_x, prev_y;
        gtk_widget_get_size_request (GTK_WIDGET (self), &prev_x, &prev_y);
        ...
        if (new_x + self->default_pane_size > prev_x) {
            ...
            g_signal_emit_by_name (self, "shrink", (gdouble) new_x + (gdouble) self->default_pane_size);
            gtk_widget_set_size_request (GTK_WIDGET (self), new_x + self->default_pane_size * 2, -1);
        }
    }
}
...
{
    if (self->n_panes == 0) {
        gtk_widget_set_size_request (GTK_WIDGET (self), self->default_pane_size * 2, -1);
        ...
        gtk_widget_set_size_request (GTK_WIDGET (new_inner_pane), self->default_pane_size, -1);
