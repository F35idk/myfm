                             ...
                             gpointer pane_scroll)
{
    ...
    g_signal_emit_by_name (GTK_SCROLLED_WINDOW (pane_scroll),
                           "scroll-child", GTK_SCROLL_STEP_LEFT,
                           TRUE, &return_val);
}

static void
mpaned_scroll_to_end_callback (MyFMMultiPaned *mpaned, gpointer pane_scroll)
{
    gboolean return_val;

    g_signal_emit_by_name (GTK_SCROLLED_WINDOW (pane_scroll),
                           "scroll-child", GTK_SCROLL_END,
                           TRUE, &return_val);
}
                                        ...
                                        gint dirview_index)
{
    ...
    /* truncate pane widget as well (function returns
     * if no directory views can be removed) */
...
myfm_window_open_other (MyFMWindow *self, MyFMFile *file)
{
    ...
    if (error) {
        myfm_utils_popup_error_dialog (self, "error in myfm_window \
...
void
myfm_window_open_file_async (MyFMWindow *self, MyFMFile *file,
                             gint dirview_index)
{
...
MyFMDirectoryView *
myfm_window_get_next_directory_view (MyFMWindow *self, MyFMDirectoryView *dirview)
{
...
gint
myfm_window_get_directory_view_index (MyFMWindow *self, MyFMDirectoryView *dirview)
{
    return g_list_index (self->directory_views, dirview);
}

static void
myfm_window_destroy (GtkWidget *widget)
{
...
static void
myfm_window_dispose (GObject *object)
{
    /* chaining up */
...
static void
myfm_window_finalize (GObject *object)
{
    MyFMWindow *self;
...
static void
myfm_window_constructed (GObject *object)
{
    MyFMWindow *self;
    ...
    gtk_container_add (GTK_CONTAINER (self->pane_scroll),
                       GTK_WIDGET (self->mpaned));
    gtk_container_add (GTK_CONTAINER (self),
                       GTK_WIDGET (self->pane_scroll));

    g_signal_connect (self->mpaned, "shrink",
                      G_CALLBACK (mpaned_scroll_left_callback),
                      self->pane_scroll);
    g_signal_connect_after (self->mpaned, "expand",
                            G_CALLBACK (mpaned_scroll_to_end_callback),
                            self->pane_scroll);
...
static void
myfm_window_class_init (MyFMWindowClass *cls)
{
...
MyFMWindow *
myfm_window_new (MyFMApplication *app)
{
